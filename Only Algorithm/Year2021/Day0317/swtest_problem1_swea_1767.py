def coloring(nx, ny, dir): # 전선이 연결 가능한 지 확인하고 전선을 연결하는 함수입니다. 인수는 좌표와 방향을 받습니다.
    if nx < 0 or ny < 0 or nx >= N or ny >= N: # 범위 끝까지 확인을 했다면 전선을 놓을 수 있는 경우 이므로
        return True
        # True를 반환합니다.
    if cells[ny][nx] != 0: # 전선을 연결하려는 공간에 코어나 전선이 있으면.
        # 즉, 전선을 놓으려는 위치가 빈 공간이 아니면
...
def dfs(x, suc):  # 현재 방향을 정할 코어의 인덱스 x와 전력 공급에 성공한 코어 개수 success의 약자 suc를 인자로 활용하겠습니다.
    global max_point, min_value # 전역변수로 선언
    if suc + (len(core) - x) < max_point: # 남은 번호의 개수가 모두 성공하더라도 최대 포인트에 미치지 못하면
        return # 가지치기
    if x == len(core): # 인덱스 x와 코어의 개수가 같다는 의미는
        count = 0 # 모든 코어에 대해 상태를 확인했을 때라는 의미이고
        for i in range(N): # 이 때 cells의 각 점을 돌며
            for j in range(N): 
                if cells[i][j] == 2: # 전선의 개수를 세도록 하겠습니다.
                    count += 1
    ...
    for i in range(4): # 4방향에 대해서
        ...
        if coloring(nx, ny, i): # 그리고 한쪽 방향으로 전선이나 코어가 있는 지 확인하고 없다면 전선을 표시합니다.
            cells[ny][nx] = 2 # 전선이 연결되므로 해당 방향에 전선을 설치합니다.
            dfs(x + 1, suc + 1) # 그리고 다음 코어에 대해서 상태를 정합니다.
            if 0 <= ny < N and 0 <= nx < N: # dfs 함수 이후에는 해당 방향의 전선을 다시 지워줘야 하므로
                cells[ny][nx] = 0 # 해당 방향의 전선과
                while 0 <= nx < N and 0 <= ny < N: # 범위 끝까지의 전선을 회수합니다.
                    cells[ny][nx] = 0 # 범위를 확인하여 반복하여야 하는 부분인데,
                    # while대신 if를 적었습니다. 오타이므로 디버깅 하면서 교체하도록 하겠습니다.
                    nx = nx + dx[i]
                    ny = ny + dy[i]
...
for tc in range(1, T + 1):
    N = int(input())
    cells = [] # 가로 n개 세로 n개의 셀을 cells로 표현하였습니다.
    for i in range(N): # n번 반복하여
        cells.append(list(map(int, input().split()))) # 각 라인의 입력값을 받아옵니다.
    core = [] # 코어의 위치를 저장하기 위한 배열
    for i in range(N): # 이중 반복을 통해 모든 cell들을 돌며 코어의 위치를 찾습니다.
        for j in range(N):
